...
contract EscrowManager is
    ...
    ReentrancyGuardUpgradeable
{
    /// @notice Order data (including state and escrow flags).
    struct OrderData {
        EscrowOrder order; // Order snapshot (buyer, seller, assets, price, deadline etc.)
        bool fundEscrowed; // Whether buyer funds are escrowed (in vault)
        bool assetEscrowed; // Whether seller target asset is escrowed (in vault/recorded)
        bool collateralEscrowed; // Whether seller collateral is escrowed (in vault)
        bytes32 disputeCaseId; // Arbitration case ID (returned by adapter after raising dispute)
        address feeRecipient; // Protocol fee recipient (snapshotted when buyer pays)
        bool buyerCreateFeePaid; // Whether buyer has paid fixed creation fee
        bool sellerCreateFeePaid; // Whether seller has paid fixed creation fee
        bool splitCreateFeeEnabled; // Whether split fixed creation fee mode is enabled (default false for old orders)
    }

    /// @notice Registry address (used to read FeeManager etc. config).
    address public registry;
    /// @notice Funds/Assets escrow vault (only callable by EscrowManager).
    EscrowVault public vault;
    /// @notice User stats contract address, can be zero to disable.
    address public userStats;
    /// @notice Number of created orders (used to generate orderId).
    uint256 public totalOrders;
    /// @notice Arbitration adapter whitelist.
    mapping(address => bool) public arbitratorWhitelist;
    /// @notice Off-chain asset (ExternalProof) minimum collateral ratio (default 100%, based on price).
    uint16 public minOffchainCollateralBps;
    /// @notice Off-chain asset minimum penalty ratio (default 100%, full slash).
    uint16 public minOffchainPenaltyBps;

    /// @notice Order data storage: orderId -> OrderData.
    mapping(bytes32 => OrderData) private orders;

    /// @notice Store all order ID list.
    bytes32[] private _allOrderIds;

    /// @notice Fee rate cap (basis points, bps): 5000 = 50%, used to prevent misconfiguration leading to high fees.
    ...
    // Event definitions are unified in IEscrowEvents.
    ...
    /// @notice Initialize contract (single contract managing multiple orders).
    ...
    /// @dev Enter paused state; only owner callable.
    function pause() external onlyOwner whenNotPaused {
        _pause();
    }

    /// @notice Unpause contract (only owner).
    /// @dev Exit paused state; only owner callable.
    ...
    /// @notice Set user stats contract address, 0 to disable user data recording.
    ...
    /// @notice Set off-chain asset minimum collateral ratio and minimum penalty ratio.
    ...
    /// @param order Order data (buyer, seller, assets, deadline etc.).
        ...
        returns (bytes32 orderId)
    {
        ...
        if (createFee > 0) {
            ...
            _transferCreateFee(createRecipient, createFee);
        } else {
        ...
        data.buyerCreateFeePaid = msg.sender == order.buyer;
        data.sellerCreateFeePaid = msg.sender == order.seller;
        data.splitCreateFeeEnabled = true;
    ...
    /// @dev Snapshot fee first, then execute escrow; state becomes Funded/AssetLocked.
    function markFunded(
        bytes32 orderId
        ...
        (
            uint256 createFeeDue,
            address createFeeRecipient
        ) = _getParticipantCreateFee(data, true);
        ...
        _escrowPayment(order, createFeeDue, createFeeRecipient);
        _markParticipantCreateFeePaid(data, true);
    ...
    /// @param orderId Order ID.
    /// @dev State becomes AssetLocked on success, may trigger auto-delivery.
    function lockAsset(
        bytes32 orderId
        ...
        (
            uint256 createFeeDue,
            address createFeeRecipient
        ) = _getParticipantCreateFee(data, false);
        if (
            order.state != OrderState.Initialized &&
            order.state != OrderState.Funded
        ) revert OmniErrors.BadState();
        ...
        if (createFeeDue > 0) {
            if (msg.value != createFeeDue) revert OmniErrors.BadCreateFee();
            _transferCreateFee(createFeeRecipient, createFeeDue);
            _markParticipantCreateFeePaid(data, false);
        } else if (msg.value != 0) {
            revert OmniErrors.UnexpectedValue();
        }
    ...
    /// @notice Seller locks collateral (Native or ERC20).
    /// @param orderId Order ID.
    /// @dev Triggers CollateralLocked event on success, may trigger auto-delivery.
    function lockCollateral(
        ...
        _escrowCollateral(order, createFeeDue, createFeeRecipient);
        _markParticipantCreateFeePaid(data, false);
    ...
    /// @notice Seller locks collateral and target asset (if any) in one go, reducing interactions.
    /// @param orderId Order ID.
    /// @dev Locks collateral and target asset as needed, triggers events.
    function lockSellerAssets(
        ...
        if (!data.assetEscrowed) {
            ...
            if (!feeHandled && createFeeDue > 0) {
                if (msg.value != createFeeDue) revert OmniErrors.BadCreateFee();
                _transferCreateFee(createFeeRecipient, createFeeDue);
                _markParticipantCreateFeePaid(data, false);
                feeHandled = true;
            } else if (!feeHandled && msg.value != 0) {
                revert OmniErrors.UnexpectedValue();
            }
            order.state = OrderState.AssetLocked;
            emit AssetLocked(orderId, msg.sender, uint64(block.timestamp));
        } else if (!feeHandled && createFeeDue > 0) {
            if (msg.value != createFeeDue) revert OmniErrors.BadCreateFee();
            _transferCreateFee(createFeeRecipient, createFeeDue);
            _markParticipantCreateFeePaid(data, false);
        } else if (!feeHandled && msg.value != 0) {
    ...
    /// @param orderId Order ID.
    /// @dev State becomes Delivered on success.
    ...
    /// @notice Buyer confirms receipt/fulfillment, executes settlement.
    /// @param orderId Order ID.
    /// @dev State becomes Completed on success, executes payout and stats/score update.
    ...
    /// @notice Cancel order before fulfillment, refund funds and return assets.
    /// @param orderId Order ID.
    /// @dev State becomes Cancelled on success, executes refund and return.
    ...
    /// @notice Handle expiration, auto-cancel or auto-complete if unfulfilled.
    /// @param orderId Order ID.
    /// @dev If delivered and funded, auto-complete; otherwise treat as cancelled.
    function expire(bytes32 orderId) external whenNotPaused nonReentrant {
        ...
        if (userStats != address(0)) {
            try UserStatsLikeV2(userStats).reputationScore() returns (
                address scoreContract
            ) {
                if (scoreContract != address(0)) {
                    int256 score = IReputationScore(scoreContract).scoreOf(
                        msg.sender
                    );
                    if (score <= 0) revert OmniErrors.ScoreTooLow();
                }
            } catch {}
        }
    ...
    /// @notice Both parties can raise a dispute, entering arbitration process.
    /// @param orderId Order ID.
    /// @param evidence Evidence CID/Raw data (off-chain storage).
    /// @dev Check state and collateral requirements, enter arbitration, record caseId and event.
    ...
    /// @notice Arbitrator rules the winner, seller win implies completion, buyer win implies cancellation.
    /// @param orderId Order ID.
    /// @param winner Address of the winner (buyer or seller).
    /// @dev Only arbitrator callable; finalizes dispute and updates state and rewards.
    ...
    /// @notice Decoupled arbitration callback.
    ...
    /// @notice Get business order value info (used for appeal fee calculation).
    /// @param orderId Business side ID (e.g., order ID).
    /// @return amount Order amount.
    /// @return token Order asset address (address(0) for native).
    /// @return assetType Asset type.
    /// @return partyA Party A (usually buyer/initiator).
    /// @return partyB Party B (usually seller/respondent).
    ...
    /// @return orderList List of corresponding order data (one-to-one with orderIds).
    function getOrders(
    ...
    /// @notice Get arbitration context (buyer/seller/price/payment asset address).
    /// @param orderId Order ID.
    ...
    /// @notice Get dispute case ID for order.
    /// @param orderId Order ID.
    ...
    /// @notice Get fee snapshot protocol fee config.
    /// @param orderId Order ID.
    /// @return recipient Protocol fee recipient.
    /// @return feeBps Dynamic discounted fee rate (bps).
    ...
    /// @param orderId Order ID.
    /// @return buyerPaid Whether buyer has paid fixed creation fee.
    /// @return sellerPaid Whether seller has paid fixed creation fee.
    /// @return splitEnabled Whether split fixed creation fee mode is enabled.
    ...
    {
        OrderData storage data = _requireOrder(orderId);
        buyerPaid = data.buyerCreateFeePaid;
        sellerPaid = data.sellerCreateFeePaid;
        splitEnabled = data.splitCreateFeeEnabled;
    }
    ...
    /// @dev Automatically advance to Delivered when on-chain target meets conditions, reducing one seller interaction.
    /// Conditions:
    /// - Not ExternalProof;
    /// - Buyer has funded;
    /// - Seller has locked target;
    /// - Collateral locked if configured;
    ...
    /// @dev Unified exit for arbitration result: update state, distribute assets and stats/score.
    /// @param orderId Order ID.
    /// @param data Order data.
    ...
    /// @dev Execute payment asset escrow (Native or ERC20), into vault.
    ...
    function _escrowPayment(
        ...
        if (order.paymentAsset.assetType == AssetType.Native) {
            if (createFeeDue > 0) {
                if (msg.value != order.paymentAsset.amount + createFeeDue)
                    revert OmniErrors.BadCreateFee();
                _transferCreateFee(createFeeRecipient, createFeeDue);
            } else if (msg.value != order.paymentAsset.amount) {
    ...
    /// @dev Escrow target asset (ERC20/ERC721/ERC1155), off-chain proof not transferred.
    ...
    /// @dev Normal completion payout: funds to seller, asset to buyer, and return collateral.
    /// @param orderId Order ID.
    /// @param data Order data.
    /// @param withArbReward Whether to distribute arbitration reward (when arbitration won).
    ...
    /// @dev Refund buyer (if funds escrowed).
    /// @param data Order data.
    ...
    /// @dev Return asset to seller (if escrowed).
    /// @param data Order data.
    ...
    /// @dev Escrow collateral asset (Native or ERC20), into vault.
    /// @param order Order struct (read collateral asset info).
    function _escrowCollateral(
        EscrowOrder storage order,
        uint256 createFeeDue,
        address createFeeRecipient
    ) internal {
        if (order.collateral.assetType == AssetType.Native) {
            if (createFeeDue > 0) {
                if (msg.value != order.collateral.amount + createFeeDue)
                    revert OmniErrors.BadCreateFee();
                _transferCreateFee(createFeeRecipient, createFeeDue);
            } else if (msg.value != order.collateral.amount) {
    ...
    /// @dev Return collateral to seller (non-arbitration payout scenario).
    /// @param orderId Order ID.
    /// @param data Order data.
    ...
    /// @dev Payout collateral to buyer based on penalty ratio, return remainder to seller.
    /// @param orderId Order ID.
    /// @param data Order data.
    ...
    /// @dev Update user stats: dispute raised.
    ...
    /// @dev Update user stats: dispute resolved.
    ...
    function _validateOrder(
        ...
        if (order.targetAsset.assetType == AssetType.ExternalProof) {
            if (order.collateral.amount == 0)
                revert OmniErrors.CollateralRequired();
            // Off-chain asset scenario: Collateral asset must match payment asset exactly (type and Token address)
    ...
    /// @dev Resolve arbitration adapter, must be whitelisted and configured in Registry.
    ...
    function _getParticipantCreateFee(
        OrderData storage data,
        bool isBuyer
    ) internal view returns (uint256 fee, address recipient) {
        if (!data.splitCreateFeeEnabled) return (0, address(0));
        if (isBuyer ? data.buyerCreateFeePaid : data.sellerCreateFeePaid) {
            return (0, address(0));
        }
        return _getCreateFeeConfig();
    }

    function _markParticipantCreateFeePaid(
        OrderData storage data,
        bool isBuyer
    ) internal {
        if (!data.splitCreateFeeEnabled) return;
        if (isBuyer) {
            data.buyerCreateFeePaid = true;
        } else {
            data.sellerCreateFeePaid = true;
        }
    }

    function _transferCreateFee(address recipient, uint256 fee) internal {
        if (fee == 0) return;
    ...
    /// @param limit Return count limit (0 means no return).
    /// @return orderIds Order ID list.
    /// @return total Total order count.
    function getOrders(
        uint256 offset,
        uint256 limit
...
interface UserStatsLikeV2 {
    function reputationScore() external view returns (address);
