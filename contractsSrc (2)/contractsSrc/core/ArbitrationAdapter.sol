    ...
    IArbitrationAdapterExtended
{
    ...
    function finalize(bytes32 caseId) external {
        ...
        if (
            ...
            address loser = (c.winner == partyA) ? partyB : partyA;
            if (msg.sender == loser) {
                isEarlyFinalize = true;
            }
        }

        if (!isEarlyFinalize && block.timestamp < c.deadline)
            revert OmniErrors.NotDue();

        if (
            c.state == CaseState.PrimaryResolved &&
            !c.finalRequested &&
            (block.timestamp >= c.appealDeadline || isEarlyFinalize)
        ) {
    ...
    function _distributeReward(
        ...
        // Determine winning vote if resolved
        Vote winningVote = Vote.None;
        if (c.resolved && c.winner != address(0)) {
            (, , , address partyA, ) = IArbitrable(c.escrow)
                .getArbitrationValue(c.orderId);
            winningVote = (c.winner == partyA) ? Vote.Buyer : Vote.Seller;
        }
        ...
        for (uint256 i = 0; i < c.jurors.length; i++) {
            if (c.votes[c.jurors[i]] != Vote.None) {
                if (
                    winningVote == Vote.None ||
                    c.votes[c.jurors[i]] == winningVote
                ) {
                    voterCount++;
                }
            }
